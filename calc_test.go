...
func TestArrayBroadcastHelpers(t *testing.T) {
	...
	t.Run("compareTypedValues_with_errors", func(t *testing.T) {
		// Test comparing error types - covers the fallback return 0 path
		lCell := newErrorFormulaArg(formulaErrorVALUE, formulaErrorVALUE)
		rCell := newErrorFormulaArg(formulaErrorVALUE, formulaErrorVALUE)
		result := compareTypedValues(lCell, rCell)
		assert.Equal(t, 0, result) // Fallback path for non-number, non-string types
	})

	t.Run("compareTypedValues_error_vs_number", func(t *testing.T) {
		// Test mixed error and number types
		lCell := newErrorFormulaArg(formulaErrorVALUE, formulaErrorVALUE)
		rCell := newNumberFormulaArg(5)
		result := compareTypedValues(lCell, rCell)
		assert.Equal(t, 0, result) // Fallback path
	})

	t.Run("compareTypedValues_error_vs_string", func(t *testing.T) {
		// Test mixed error and string types
		lCell := newErrorFormulaArg(formulaErrorDIV, formulaErrorDIV)
		rCell := newStringFormulaArg("test")
		result := compareTypedValues(lCell, rCell)
		assert.Equal(t, 0, result) // Fallback path
	})
...
func TestEmptyMatrixEdgeCases(t *testing.T) {
	...
	t.Run("matrix_with_zero_columns_in_eq_operation_right", func(t *testing.T) {
		// Test empty columns matrix as right operand in comparison
		lOpd := newNumberFormulaArg(5)
		rMatrix := [][]formulaArg{{}} // One row, zero columns
		rOpd := newMatrixFormulaArg(rMatrix)
		...
		result := opdStack.Pop().(formulaArg)
		assert.Equal(t, ArgError, result.Type)
		assert.Equal(t, formulaErrorVALUE, result.Value())
	})

	t.Run("matrix_with_zero_columns_in_eq_operation_left", func(t *testing.T) {
		// Test empty columns matrix as left operand in comparison
		lMatrix := [][]formulaArg{{}} // One row, zero columns
		lOpd := newMatrixFormulaArg(lMatrix)
		rOpd := newNumberFormulaArg(5)
...
func TestEmptyMatrixNoRegression(t *testing.T) {
	...
	t.Run("scalar_eq_comparison_still_works", func(t *testing.T) {
		// Test scalar equality after array check was added
		lScalar := newNumberFormulaArg(5)
		rScalar := newNumberFormulaArg(5)

		opdStack := NewStack()
		err := calcEq(rScalar, lScalar, opdStack)
		assert.NoError(t, err)
	...
	t.Run("scalar_neq_comparison_still_works", func(t *testing.T) {
		// Test scalar not-equal after array check was added
		lScalar := newNumberFormulaArg(5)
		rScalar := newNumberFormulaArg(3)

		opdStack := NewStack()
		err := calcNEq(rScalar, lScalar, opdStack)
		assert.NoError(t, err)
	...
	t.Run("scalar_less_than_comparison_still_works", func(t *testing.T) {
		// Test scalar less-than after array check was added
		lScalar := newNumberFormulaArg(3)
		rScalar := newNumberFormulaArg(5)

		opdStack := NewStack()
		err := calcL(rScalar, lScalar, opdStack)
		assert.NoError(t, err)
	...
	t.Run("scalar_greater_than_comparison_still_works", func(t *testing.T) {
		// Test scalar greater-than after array check was added
		lScalar := newNumberFormulaArg(10)
		rScalar := newNumberFormulaArg(5)

		opdStack := NewStack()
		err := calcG(rScalar, lScalar, opdStack)
		assert.NoError(t, err)
	...
	t.Run("scalar_lte_comparison_still_works", func(t *testing.T) {
		// Test scalar less-than-or-equal after array check was added
		lScalar := newNumberFormulaArg(5)
		rScalar := newNumberFormulaArg(5)

		opdStack := NewStack()
		err := calcLe(rScalar, lScalar, opdStack)
		assert.NoError(t, err)
	...
	t.Run("scalar_gte_comparison_still_works", func(t *testing.T) {
		// Test scalar greater-than-or-equal after array check was added
		lScalar := newNumberFormulaArg(5)
		rScalar := newNumberFormulaArg(5)

		opdStack := NewStack()
		err := calcGe(rScalar, lScalar, opdStack)
		assert.NoError(t, err)
